    ...
    <iv-row>
      <iv-col :xs="24" :sm="24" :md="24" :lg="17">
        ...
        >
          ...
          <iv-formItem prop="title" label="文章标题：">
            <iv-input
              placeholder="请填写文章标题..."
              :maxlength="40"
          ...
          <iv-formItem prop="description" label="文章描述：">
            <iv-input
              type="textarea"
              placeholder="描述一下你文章的主要内容把~"
          ...
          <iv-formItem prop="content" label="文章内容：">
            <mavon-editor
              ref="md"
              @imgAdd="imgAdd"
          ...
          <iv-formItem label="文章类型：">
            <iv-select v-model="article.status" style="width: 200px">
              <iv-option
        ...
        <iv-button type="error" @click="saveArticle('article')">发表文章</iv-button>
      </iv-col>
    ...
    saveArticle: function (formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.$axios.post('/article/insert',article)
          .then(({ data }) => {
            if (data && data.code === 1) {
              console.log("------------------",data)
              this.$Notice.success({
              ...
              // 跳转到list
              this.$router.push("/article/"+data.data.keyId);
            } else {
              this.$Notice.success({
                title: '提示',
                desc: data.msg
