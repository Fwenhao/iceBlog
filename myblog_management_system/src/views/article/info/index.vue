<template>
  <d2-container>
    ...
    <el-table :data="tableData" style="width: 100%">
      <el-table-column label="文章id" prop="keyId" width="180" align="center">
      </el-table-column>
      ...
      <el-table-column label="标签" prop="tags" width="180" align="center">
      </el-table-column>
      <el-table-column
        label="浏览时间"
        prop="createTime"
...
export default {
  ...
  methods: {
    ...
    init() {
      let params = {
        articleName: this.pageInfo.name === undefined ? "" : this.pageInfo.name,
        pageSize: this.pageInfo.size,
        current: this.pageInfo.current,
      };
      console.log("params", params);
      this.$api.getArticleByAuthor(params).then((res) => {
        if (res) {
          console.log(res);
    ...
    //查看函数
    handleQuery(row) {
      window.open("http://localhost:8002/article/" + row.keyId);
    },
    //删除函数
    handleDelete(row) {
      let res = this.$confirm("此操作将永久删除该篇文章, 是否继续?", "提示", {
        ...
        .then((action) => {
          if (action === "confirm") {
            ...
            this.$api.deleteArticle(param).then((res) => {
              if (res) {
                this.$message({
                  type: "success",
                  message: "删除成功!",
