<template>
  <d2-container>
    ...
    <el-table :data="tableData" style="width: 100%">
      <el-table-column label="文章id" prop="articleId" width="180" align="center">
      </el-table-column>
      <el-table-column label="评论内容" prop="content" width="180" align="center">
      </el-table-column>
      <el-table-column
        label="评论时间"
        prop="createTime"
...
export default {
  ...
  methods: {
    ...
    init() {
      let params = {
        comment: this.pageInfo.name,
        pageSize: this.pageInfo.size,
        current: this.pageInfo.current,
      };
      console.log("params", params);
      this.$api.getCommentByUser(params).then((res) => {
        if (res) {
          console.log(res);
      ...
      this.$confirm("此操作将永久删除该评论, 是否继续?", "提示", {
        ...
        .then((action) => {
          if (action === "confirm") {
            ...
            this.$api.deleteComment(param).then((res) => {
              if (res) {
                this.$message({
                  type: "success",
                  message: "删除成功!",
